...
当需要补日志时，优先使用 `Scenario Implementation Agent` 作为执行型子 agent，并在 prompt 中包含以下要求：
- 先读取 `/autoware/.github/agents/scenario-node-debug-planner.agent.md`
...
当出现以下任一情况时，在当前对话内调用子 agent 执行补日志，而不是 handoff：
- 需要在代码里补日志才能继续判断
- 现有日志无法区分多个候选原因
- 需要沿调用链往下定位关键节点
- 需要把当前仿真结果转成可执行的日志插入动作
